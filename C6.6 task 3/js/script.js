...
function openWebsocket() {
  ...
  websocket.onmessage = function(e) {
    if (e.data !== msgGeolocation) {
      ...
      writeToScreen(`<span  
                          style="color: blue; display: block; width: 45%; \
                          text-align: left; border: solid; \
                          border-color: lightblue;">` + e.data + '</span>');
    } 
  };
  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      writeToScreen(`<span  
                          ...
                          border-color: lightblue;">` + msgGeolocation + 
...
btnSendNode.addEventListener('click', () => {
  ...
  if (valueInputMsg!=='') {
    ...
    writeToScreen(`<span  
                          style="color: black; display: block; text-align: right; \
                          border: solid; width: 45%; margin-left: 270px; \
                          border-color: lightblue;">` + valueInputMsg + '</span>');
